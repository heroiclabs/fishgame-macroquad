...
function resize(canvas, on_resize) {
    ...
    if (canvas.width != displayWidth ||
        ...
        if (displayWidth > 800 || displayWidth > 600) {
            canvas.width = 800;
            canvas.height = 600;
        } else {
        ...
        if (on_resize != undefined)
            on_resize(Math.floor(canvas.width), Math.floor(canvas.height))
    }
}
...
function mouse_relative_position(clientX, clientY) {
    ...
    var x = (clientX - targetRect.left) * canvas.width / canvas.clientWidth;
    var y = (clientY - targetRect.top) * canvas.height / canvas.clientHeight;
